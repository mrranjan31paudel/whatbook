...
class ProfileEditPopUp extends React.Component {
    ...
    if (editedItem === 'name') {
      ...
      if (this.state.profileEditData.currentPassword) {
        if (this.validatePassword()) {
          this.props.onPasswordChangeSubmit({
  ...
  validatePassword = () => {
    if (this.state.profileEditData.newPassword.length > 5 && this.state.profileEditData.confirmPassword.length > 5 && this.state.profileEditData.confirmPassword === this.state.profileEditData.newPassword) {
      return true;
    }
    return false;
  }
              ...
              <div className="profile-edit-field-wrapper">
                <label className="profile-edit-field-label">Name: </label>
                {
                  this.state.clickedEditButton === 'edit-name-button' ?
                    <input className="profile-edit-input-field" type="text" name="user-name" defaultValue={this.props.userData.name} onChange={this.handleNameChange} autoFocus>
                    </input> :
              ...
              <div className="profile-edit-field-wrapper">
                <label className="profile-edit-field-label">DOB: </label>
                {
                  this.state.clickedEditButton === 'edit-dob-button' ?
                    <input className="profile-edit-input-field" type="date" defaultValue={formatToYYYYMMDD(this.props.userData.dob)} onChange={this.handleDobChange}>
                    </input> :
                ...
                <label className="profile-edit-field-label">Password: </label>
                {
                  this.state.clickedEditButton === 'edit-password-button' ?
                    <div className="password-edit-field-container">
                      <input
                        className={this.state.wasPasswordWrong ? 'profile-edit-input-field current-password-wrong' : 'profile-edit-input-field'}
                      ...
                      </input>
                      {
                        this.state.wasPasswordWrong ?
                          <label className="password-wrong-label">Wrong Password!</label> : ''
                      }
                      <input className="profile-edit-input-field" type="password" defaultValue="" placeholder="New Password" onChange={this.handleNewPasswordChange}>
                      </input>
                      {
                        this.state.profileEditData.newPassword.length > 0 && this.state.profileEditData.newPassword.length < 6 ?
                          <label className="password-length-constraint-label">At least 6 characters!</label> : ''
                      }
                      <input
                        className={this.validatePassword() ? 'profile-edit-input-field password-change-match' : 'profile-edit-input-field password-change-not-match'}
                        type="password"
                        defaultValue=""
                ...
                <div className="edit-action-buttons">
                  {
                    this.state.clickedEditButton === 'edit-password-button' ?
                      <Fragment>
                        <button id="save-password-button" onClick={this.handleEditSubmit}>Save</button>
                        <button id="cancel-password-button" onClick={this.handleEditCancelClick}>Cancel</button>
                      </Fragment> :
                      <Fragment>
                        {
                          this.state.passwordChangeSuccess ?
                            <label className="password-successfully-changed-label">Password Successfully Changed</label> :
                            <button id="edit-password-button" onClick={this.handleFieldEditButtonClick} >Change</button>
                        }
                      </Fragment>
                  }
                </div>
